...
class TestGraphStructure:
    ...
    def test_entry_point(self):
        graph = agent_graph.get_graph()
        # entry point is __start__ → init
        edges = list(graph.edges)
        start_edges = [e for e in edges if e.source == "__start__"]
        assert len(start_edges) == 1
        assert start_edges[0].target == "init"

    def test_conditional_from_plan(self):
        graph = agent_graph.get_graph()
        edges = list(graph.edges)
        cond = [e for e in edges if e.source == "plan" and e.conditional]
        assert len(cond) == 2  # exec_task and synthesize
